...
import { Info, BarChart, Landmark, ArrowUpRight } from "lucide-react";
...
// Mock Data for Area Chart (Gross vs Net)
const data = Array.from({ length: 21 }).map((_, i) => {
    const year = i;
    const initial = 100000;
    const annualAddition = 24000;
    const returnRate = 0.08;
    const taxRate = 0.15; // 15% estimated long-term capital gains tax

    // Simplistic compounding model
    const grossFV = initial * Math.pow(1 + returnRate, year) + annualAddition * ((Math.pow(1 + returnRate, year) - 1) / returnRate);
    const totalBasis = initial + (annualAddition * year);
    const grossProfit = grossFV - totalBasis;
    const netFV = totalBasis + (grossProfit * (1 - taxRate));

    return {
        year: `Year ${year}`,
        gross: Math.round(grossFV),
        net: Math.round(netFV),
        basis: Math.round(totalBasis)
    };
});
...
export default function InvestmentReturnsCalculator() {
    return (
        <div className="min-h-screen relative overflow-hidden bg-gradient-to-b from-[#090B14] to-black">
            {/* Ambient Deep Space Grid & Volumetric Glows (Emerald hue for wealth) */}
            ...
            <div className="relative border-b border-white/[0.04] py-16 backdrop-blur-sm z-10">
                <div className="container mx-auto px-4 max-w-7xl relative z-10">
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <h1 className="text-4xl md:text-5xl lg:text-7xl font-black text-white tracking-tighter drop-shadow-lg">Capital Allocation Returns.</h1>
                    </div>
                    <p className="text-slate-400 max-w-2xl font-light leading-relaxed">
                        After-tax portfolio modeling. Forecast net yield and calculate the drag of capital gains taxation on long-term compound systems.
                    </p>
                </div>
            </div>
                    ...
                    <div className="lg:col-span-4 relative z-10">
                        <Card className="bg-gradient-to-b from-white/[0.03] to-transparent h-full border-t border-emerald-500/10">
                            ...
                            <h2 className="text-xs font-bold text-emerald-400 uppercase tracking-widest mt-2 mb-6 border-b border-white/[0.04] pb-4 flex items-center gap-2">
                                <Landmark className="w-4 h-4" /> Investment Core
                            </h2>

                            <Input label="Initial Capital Deployed" value="100,000" prefix="$" bg="bg-black/40" />
                            <Input label="Annual Capital Addition" value="24,000" prefix="$" bg="bg-black/40" />
                            ...
                            <h2 className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest mt-8 mb-6 border-b border-white/[0.04] pb-4">
                                Time & Taxation Vectors
                            </h2>

                            <Input label="Investment Length" value="20" suffix="Years" bg="bg-black/40" />
                            <Input label="Expected Capital Gains Tax Rate" value="15.0" suffix="%" bg="bg-white/[0.02] text-slate-400" />
                            ...
                            <button className="relative z-10 w-full bg-emerald-600/90 text-white py-4 mt-8 text-sm font-bold tracking-wide hover:bg-emerald-500 transition-colors shadow-[0_0_20px_rgba(16,185,129,0.2)] rounded-lg flex items-center justify-center gap-2">
                                RUN TAX-ADJUSTED FORECAST <ArrowUpRight className="w-4 h-4" />
                            </button>
                        </Card>
                    </div>
                    ...
                    <div className="lg:col-span-8 flex flex-col gap-8 relative z-10">
                        ...
                        <div className="bg-[#05060A]/80 border border-emerald-500/20 p-8 sm:p-12 relative overflow-hidden rounded-2xl shadow-[0_0_40px_rgba(16,185,129,0.1)] backdrop-blur-3xl">
                            ...
                            <div className="relative z-10 grid md:grid-cols-2 gap-12 items-end">
                                <div>
                                    <h3 className="text-slate-400 text-xs tracking-widest uppercase font-medium mb-4">Final Value (Net)</h3>
                                    <div className="text-6xl sm:text-7xl font-mono font-bold text-white tracking-tighter drop-shadow-2xl">
                                        <span className="text-4xl text-emerald-500/80 mr-1">$</span>1<span className="text-5xl text-white">.49M</span>
                                    </div>
                                    <p className="text-[10px] font-mono mt-4 text-emerald-400/80 uppercase tracking-widest bg-emerald-500/10 inline-block px-2 py-1 rounded">Equivalent to $1,493,524 Realized Capital</p>
                                </div>
                                <div className="space-y-6 border-t md:border-t-0 md:border-l border-white/[0.04] pt-8 md:pt-0 md:pl-12">
                                    <div className="bg-[#05060A]/90 backdrop-blur-xl p-6 rounded-xl border border-white/[0.04] shadow-inner mb-4 flex flex-col justify-center h-[90px]">
                                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Gross Terminal Value</p>
                                        <p className="text-2xl font-mono text-slate-300">$1,632,382</p>
                                    </div>
                                    <div className="bg-[#05060A]/90 backdrop-blur-xl p-6 rounded-xl border border-rose-500/20 shadow-[0_0_20px_rgba(244,63,94,0.1)] flex flex-col justify-center h-[90px]">
                                        <p className="text-[10px] font-bold text-rose-500 uppercase tracking-widest mb-1">Total Estimated Tax Drag</p>
                                        <p className="text-2xl font-mono text-rose-400">-$138,858</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <Card>
                            <div className="flex items-center justify-between mb-8 pb-4 border-b border-white/[0.04]">
                                <h3 className="text-sm font-bold text-white uppercase tracking-widest flex items-center gap-2">
                                    Gross vs. Net Efficiency Trajectory
                                    <Info className="w-4 h-4 text-slate-500" />
                                </h3>
                                <div className="flex gap-4">
                                    <div className="flex items-center gap-2 text-[10px] uppercase font-bold text-slate-400"><div className="w-2 h-2 bg-slate-600 rounded-full"></div> Capital Basis</div>
                                    <div className="flex items-center gap-2 text-[10px] uppercase font-bold text-slate-400"><div className="w-2 h-2 bg-emerald-700/50 rounded-full"></div> Gross Balance</div>
                                    <div className="flex items-center gap-2 text-[10px] uppercase font-bold text-emerald-400"><div className="w-2 h-2 bg-emerald-400 rounded-full shadow-[0_0_8px_rgba(16,185,129,0.5)]"></div> Net Position (After Tax)</div>
                                </div>
                            </div>
                            <div className="h-[300px] w-full mt-4">
                                <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1} aspect={undefined}>
                                    <AreaChart data={data} margin={{ top: 10, right: 0, left: 0, bottom: 0 }}>
                                        ...
                                        />
                                        <Area type="monotone" dataKey="gross" stroke="#047857" strokeWidth={1} fill="transparent" strokeDasharray="3 3" />
                                        <Area type="monotone" dataKey="net" stroke="#10B981" strokeWidth={3} fill="#10B981" fillOpacity={0.15} />
                                        <Area type="monotone" dataKey="basis" stroke="#475569" strokeWidth={2} fill="transparent" strokeDasharray="5 5" />
                                    </AreaChart>
                                </ResponsiveContainer>
                            </div>
                        </Card>
