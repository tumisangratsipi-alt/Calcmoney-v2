...
import { Info, Target, Coins, ArrowUpRight } from "lucide-react";
...
// Mock Data for Area Chart (Savings Progress)
const data = Array.from({ length: 6 }).map((_, i) => {
    const year = i;
    const endYear = 5;
    const target = 50000;
    const monthlyNeeded = 721.45;

    // Simplistic progression to target
    const currentVal = target * (year / endYear);
...
export default function SavingsGoalCalculator() {
    return (
        <div className="min-h-screen relative overflow-hidden bg-gradient-to-b from-[#090B14] to-black">
            {/* Ambient Deep Space Grid & Volumetric Glows (Emerald hue for target acquisition) */}
            ...
            <div className="relative border-b border-white/[0.04] py-16 backdrop-blur-sm z-10">
                <div className="container mx-auto px-4 max-w-7xl relative z-10">
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <h1 className="text-4xl md:text-5xl lg:text-7xl font-black text-white tracking-tighter drop-shadow-lg">Target Acquisition Engine.</h1>
                    </div>
                    <p className="text-slate-400 max-w-2xl font-light leading-relaxed">
                        Precision savings modeling. Determine the exact monthly liquidity required to capture your financial targets on schedule.
                    </p>
                </div>
            </div>
                    ...
                    <div className="lg:col-span-4 relative z-10">
                        <Card className="bg-gradient-to-b from-white/[0.03] to-transparent h-full border-t border-emerald-500/10">
                            ...
                            <h2 className="text-xs font-bold text-emerald-400 uppercase tracking-widest mt-2 mb-6 border-b border-white/[0.04] pb-4 flex items-center gap-2">
                                <Target className="w-4 h-4" /> Goal Parameters
                            </h2>

                            <Input label="Target Amount (Goal)" value="50,000" prefix="$" bg="bg-black/40" />
                            <Input label="Starting Balance" value="5,000" prefix="$" bg="bg-black/40" />

                            <h2 className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest mt-8 mb-6 border-b border-white/[0.04] pb-4">
                                Strategic Vectors
                            </h2>

                            <Input label="Time Horizon" value="5" suffix="Years" bg="bg-black/40" />
                            <Input label="Expected Annual Return" value="4.5" suffix="%" bg="bg-white/[0.02] text-slate-400" />
                            ...
                            <button className="relative z-10 w-full bg-emerald-600/90 text-white py-4 mt-8 text-sm font-bold tracking-wide hover:bg-emerald-500 transition-colors shadow-[0_0_20px_rgba(16,185,129,0.2)] rounded-lg flex items-center justify-center gap-2">
                                LOCK TARGET <ArrowUpRight className="w-4 h-4" />
                            </button>
                        </Card>
                    </div>
                    ...
                    <div className="lg:col-span-8 flex flex-col gap-8 relative z-10">
                        ...
                        <div className="bg-[#05060A]/80 border border-emerald-500/20 p-8 sm:p-12 relative overflow-hidden rounded-2xl shadow-[0_0_40px_rgba(16,185,129,0.1)] backdrop-blur-3xl">
                            ...
                            <p className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest mb-4 flex items-center gap-3 relative z-10 border-b border-white/[0.04] pb-4">
                                ...
                                Monthly Liquidity Required
                            </p>

                            <div className="relative z-10 grid md:grid-cols-2 gap-12 items-end">
                                <div>
                                    <h3 className="text-slate-400 text-xs tracking-widest uppercase font-medium mb-4">Required Monthly Savings</h3>
                                    <div className="text-6xl sm:text-7xl font-mono font-bold text-white tracking-tighter drop-shadow-2xl">
                                        <span className="text-4xl text-emerald-500/80 mr-1">$</span>721<span className="text-3xl text-slate-500">.45</span>
                                    </div>
                                    <p className="text-[10px] font-mono mt-4 text-emerald-400/80 uppercase tracking-widest bg-emerald-500/10 inline-block px-2 py-1 rounded">Based on 5 Year Trajectory</p>
                                </div>
                                <div className="space-y-6 border-t md:border-t-0 md:border-l border-white/[0.04] pt-8 md:pt-0 md:pl-12">
                                    <div className="bg-[#05060A]/90 backdrop-blur-xl p-6 rounded-xl border border-emerald-500/20 shadow-[0_0_20px_rgba(16,185,129,0.1)] mb-4">
                                        <p className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest mb-2">Total Contributions Required</p>
                                        <p className="text-3xl font-mono text-emerald-400">$43,287</p>
                                    </div>
                                    <div className="bg-[#05060A]/90 backdrop-blur-xl p-6 rounded-xl border border-white/[0.04] shadow-inner">
                                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Total Interest Projected</p>
                                        <p className="text-3xl font-mono text-white">$6,713</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <Card>
                            <div className="flex items-center justify-between mb-8 pb-4 border-b border-white/[0.04]">
                                ...
                                <div className="flex gap-4">
                                    <div className="flex items-center gap-2 text-[10px] uppercase font-bold text-slate-400"><div className="w-2 h-2 bg-emerald-400 rounded-full shadow-[0_0_8px_rgba(16,185,129,0.5)]"></div> Projected Balance</div>
                                    <div className="flex items-center gap-2 text-[10px] uppercase font-bold text-slate-400"><div className="w-2 h-2 bg-rose-500 rounded-full"></div> Capital Target</div>
                                </div>
                            </div>
                            <div className="h-[250px] w-full mt-4">
                                <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1} aspect={undefined}>
                                    <AreaChart data={data} margin={{ top: 10, right: 0, left: 0, bottom: 0 }}>
                                        ...
                                        />
                                        <ReferenceLine y={50000} stroke="#F43F5E" strokeDasharray="3 3" label={{ position: 'top', value: 'TARGET: $50K', fill: '#F43F5E', fontSize: 10, fontWeight: 'bold' }} />
                                        <Area type="monotone" dataKey="balance" stroke="#10B981" strokeWidth={3} fill="#10B981" fillOpacity={0.15} />
                                    </AreaChart>
                                </ResponsiveContainer>
                            </div>
                        </Card>
