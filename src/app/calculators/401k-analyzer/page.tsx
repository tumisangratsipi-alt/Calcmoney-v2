...
import { Info, Briefcase, TrendingUp, ArrowUpRight } from "lucide-react";
...
const data = Array.from({ length: 35 }).map((_, i) => {
    ...
    const compoundPrincipal = initial * Math.pow(1 + rate, i);
    const compoundContributions = (annualContribution + match) * ((Math.pow(1 + rate, i) - 1) / rate);
    const balance = compoundPrincipal + compoundContributions;
...
export default function Analyzer401k() {
    ...
    return (
        <div className="min-h-screen relative overflow-hidden bg-gradient-to-b from-[#090B14] to-black">
            {/* Ambient Deep Space Grid & Volumetric Glows (Teal hue for wealth) */}
            ...
            <div className="relative border-b border-white/[0.04] py-16 backdrop-blur-sm z-10">
                <div className="container mx-auto px-4 max-w-7xl relative z-10">
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <h1 className="text-4xl md:text-5xl lg:text-7xl font-black text-white tracking-tighter drop-shadow-lg">401k Optimization Engine.</h1>
                    </div>
                    <p className="text-slate-400 max-w-2xl font-light leading-relaxed">
                        Retirement architecture modeling. Maximize employer match utilization and map compounding trajectories to exit age.
                    </p>
                </div>
            </div>
            ...
            <div className="container mx-auto px-4 py-12 max-w-7xl relative z-10">
                <div className="grid lg:grid-cols-12 gap-8 lg:gap-12">
                    ...
                    <div className="lg:col-span-4 relative z-10">
                        <Card className="bg-gradient-to-b from-white/[0.03] to-transparent h-full border-t border-teal-500/10">
                            ...
                            <h2 className="text-xs font-bold text-teal-400 uppercase tracking-widest mt-2 mb-6 border-b border-white/[0.04] pb-4 flex items-center gap-2">
                                <TrendingUp className="w-4 h-4" /> Trajectory Parameters
                            </h2>

                            <Input label="Current Age" value={currentAge} onChange={(e) => setCurrentAge(Number(e.target.value))} bg="bg-black/40" />
                            <Input label="Retirement Target Age" value={retirementAge} onChange={(e) => setRetirementAge(Number(e.target.value))} bg="bg-black/40" />
                            <Input label="Current 401(k) Balance" value="50,000" prefix="$" bg="bg-black/40" />

                            <h2 className="text-[10px] font-bold text-teal-400 uppercase tracking-widest mt-8 mb-6 border-b border-white/[0.04] pb-4">
                                Contribution Mechanics
                            </h2>

                            <Input label="Est. Annual Contribution" value="12,000" prefix="$" bg="bg-black/40" />
                            <Input label="Employer Match (Effective)" value="50" suffix="%" bg="bg-black/40" />
                            <Input label="Expected Annual Return" value="7.0" suffix="%" bg="bg-white/[0.02] text-slate-400" />

                            <button className="relative z-10 w-full bg-teal-600/90 text-white py-4 mt-8 text-sm font-bold tracking-wide hover:bg-teal-500 transition-colors shadow-[0_0_20px_rgba(20,184,166,0.2)] rounded-lg flex items-center justify-center gap-2">
                    ...
                    <div className="lg:col-span-8 flex flex-col gap-8 relative z-10">
                        ...
                        <div className="bg-[#05060A]/80 border border-teal-500/20 p-8 sm:p-12 relative overflow-hidden rounded-2xl shadow-[0_0_40px_rgba(20,184,166,0.1)] backdrop-blur-3xl">
                            ...
                            <p className="text-[10px] font-bold text-teal-400 uppercase tracking-widest mb-4 flex items-center gap-3 relative z-10 border-b border-white/[0.04] pb-4">
                                <div className="w-2 h-2 rounded-full bg-teal-400 animate-pulse shadow-[0_0_15px_rgba(20,184,166,0.8)]"></div>
                                Retirement Projection
                            </p>

                            <div className="relative z-10 grid md:grid-cols-2 gap-12 items-end">
                                <div>
                                    <h3 className="text-slate-400 text-xs tracking-widest uppercase font-medium mb-4">Projected Balance at 65</h3>
                                    <div className="text-6xl sm:text-7xl font-mono font-bold text-white tracking-tighter drop-shadow-2xl">
                                        <span className="text-4xl text-teal-500/80 mr-1">$</span>3<span className="text-5xl text-white">.01M</span>
                                    </div>
                                    <p className="text-[10px] font-mono mt-4 text-teal-400/80 uppercase tracking-widest bg-teal-500/10 inline-block px-2 py-1 rounded">35 Years of Compound Growth</p>
                                </div>
                                <div className="space-y-6 border-t md:border-t-0 md:border-l border-white/[0.04] pt-8 md:pt-0 md:pl-12">
                                    <div className="bg-[#05060A]/90 backdrop-blur-xl p-6 rounded-xl border border-white/[0.04] shadow-inner mb-4">
                                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Total Personal Contributions</p>
                                        <p className="text-3xl font-mono text-white">$420,000</p>
                                    </div>
                                    <div className="bg-[#05060A]/90 backdrop-blur-xl p-6 rounded-xl border border-teal-500/20 shadow-[0_0_20px_rgba(20,184,166,0.1)]">
                                        <p className="text-[10px] font-bold text-amber-500 uppercase tracking-widest mb-2">Total Employer Match</p>
                                        <p className="text-3xl font-mono text-amber-400">$210,000</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <Card>
                            <div className="flex items-center justify-between mb-8 pb-4 border-b border-white/[0.04]">
                                <h3 className="text-sm font-bold text-white uppercase tracking-widest flex items-center gap-2">
                                    Exponential Compounding Matrix
                                    <Info className="w-4 h-4 text-slate-500" />
                                </h3>
                                <div className="flex gap-4">
                                    <div className="flex items-center gap-2 text-[10px] uppercase font-bold text-slate-400"><div className="w-2 h-2 bg-teal-400 rounded-full shadow-[0_0_8px_rgba(20,184,166,0.5)]"></div> Portfolio Value</div>
                                </div>
                            </div>
                            <div className="h-[300px] w-full mt-4">
                                <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1} aspect={undefined}>
                                    <AreaChart data={data} margin={{ top: 10, right: 0, left: 0, bottom: 0 }}>
                                        <XAxis dataKey="age" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#64748b' }} dy={10} minTickGap={2} />
                                        <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#64748b' }} tickFormatter={(value) => `$${value / 1000000}M`} dx={-10} />
                                        <RechartsTooltip
                                        ...
                                        />
                                        <Area type="monotone" dataKey="balance" stroke="#0D9488" strokeWidth={3} fill="#14B8A6" fillOpacity={0.15} />
                                    </AreaChart>
                                </ResponsiveContainer>
                            </div>
                        </Card>
